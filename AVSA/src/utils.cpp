...
std::vector<Rect> readGroundTruthFile(std::string groundtruth_path)
{
	...
	while(getline(inFile, bbox_values)){
		...
		bbox_list.push_back(Rect((int)xmin, (int)ymin, (int)width, (int)height));
...
cv::Mat		CalculateHistogram (const cv::Mat& patch, int histSize, int nrBins)
{
	...
	Mat hist;
	calcHist (&patch, 1, 0, Mat (), hist, 1, &nrBins, &histRange, uniform, accumulate);
	return hist;
}

Mat		CreateHistogramImage (std::string title, const cv::Mat& hist, int width /*= 400*/)
{
	int hist_w = width; 
	int hist_h = 200;
	int bin_w = cvRound ((double)hist_w / hist.rows);
	Mat histImage (hist_h, hist_w, CV_8UC1, Scalar (0));
	Mat histogram = hist * histImage.rows;	
	...
	putText (histImage, title, Point (5, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar (255, 255, 255));
