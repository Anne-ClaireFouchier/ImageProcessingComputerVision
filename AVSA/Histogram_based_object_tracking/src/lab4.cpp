 ...
 */
//includes
#include <iostream>//#include <stdio.h> 								//Standard I/O library
...
int main (int argc, char** argv)
{
	...
	std::string dataset_path = "/home/luni/eclipse-workspace/Lab4/dataset";									//dataset location.
	std::string output_path = "/home/luni/eclipse-workspace/Lab4/dataset/outvideos/";							//location to save output videos
	...
	constexpr unsigned int nrBinsColor		= 16;
	constexpr unsigned int nrBinsGradient	= 9;
	constexpr unsigned int nrCandidates		= 225;
	constexpr unsigned int stride			= 2;
	constexpr char channel					= 'R'; // R,G,B,H,S
	constexpr char model					= 'C'; // C, G, F
	...
	for (int s = 0; s < NumSeq; s++) {
		...
		cv::Size frame_size ((int)cap.get (cv::CAP_PROP_FRAME_WIDTH), (int)cap.get (cv::CAP_PROP_FRAME_HEIGHT));//cv::Size frame_size(700,460);
		VideoWriter outputvideo (output_path + "outvid_" + sequences[s] + ".avi", CV_FOURCC ('X', 'V', 'I', 'D'), 10, frame_size);	//xvid compression (cannot be changed in OpenCV) // // change before submit
		...
		for (;;) {
			...
			frame_idx = (int)cap.get (cv::CAP_PROP_POS_FRAMES);			//get the current frame
			...
			if (frame_idx == 1) {
				switch (model) {
					case 'C':
						tracker = new Tracker (new ColorModel (frame, list_bbox_gt[0], nrBinsColor, channel), nrCandidates, stride);
						break;
					case 'G':
						tracker = new Tracker (new GradientModel (frame, list_bbox_gt[0], nrBinsGradient), nrCandidates, stride);
						break;
					case 'F':
						tracker = new Tracker (new FusionModel (frame, list_bbox_gt[0], nrBinsColor, nrBinsGradient, channel), nrCandidates, stride);
						break;
				}
				list_bbox_est.push_back (list_bbox_gt[0]);
				continue;
			}
			list_bbox_est.push_back (tracker->GetEstimate (frame));
		...
		//imshow ("Scores", CreateHistogramImage ("Scores", Mat (trackPerf), 700));
		//waitKey ();
		...
		std::cout << "  Average tracking performance = " << std::accumulate (trackPerf.begin (), trackPerf.end (), 0.0) / trackPerf.size () << std::endl << endl;
